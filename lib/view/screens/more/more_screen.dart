              ...
              return Column( mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                Column(
                  ...
                  children: [
                    ...
                    MenuItem(image: Images.user_icon, title: _isLoggedIn ? 'profile'.tr : 'guest'.tr, tagText: _isLoggedIn ? (profileController.profileImageUrl == '' || profileController.profileImageUrl == null) ?'50 %' : '100 %' : '' ,
                    ...
                    MenuItem(image: Images.saved_address_icon, title: 'saved_address'.tr, tagText: "0", isTagActive: false,
                        onTap: () { Get.toNamed(RouteHelper.getSavedAddressRoute()); }),

                    MenuItem(image: Images.my_offer_icon, title: 'coupons'.tr, onTap: () { Get.toNamed(RouteHelper.getCouponRoute( false )); }),

                    // MenuItem(image: Images.payment, title: 'payment'.tr, onTap: () {}),

                    MenuItem(image: Images.settings, title: 'settings'.tr, onTap: () => Get.toNamed(RouteHelper.getSettingsRoute())),

                    MenuItem(image: Images.terms_and_condition, title: 'terms_conditions'.tr, onTap: () {  Get.toNamed(RouteHelper.getHtmlRoute('terms-and-condition')); }),

                    MenuItem(image: Images.privacy_policy, title: 'privacy_policy'.tr, onTap: () { Get.toNamed( RouteHelper.getHtmlRoute('privacy-policy')); }),

                    MenuItem(image: Images.about_us, title: 'about_us'.tr, onTap: () {  Get.toNamed( RouteHelper.getHtmlRoute('about_us')); }),
                  ],
                ),
                ...
                _isLoggedIn ? InkWell(
                  onTap: () {
                    if(Get.find<AuthController>().isLoggedIn()) {
                      Get.dialog(ConfirmationDialog(icon: Images.delete_account_dialog, description: 'are_you_sure_to_delete_account'.tr, isLogOut: true, onYesPressed: () async {
                        await Get.find<AuthController>().deleteToken();
                        await Get.find<ProfileController>().removeUser();
                      ...
                      children: [
                        Padding(
                          padding: EdgeInsets.symmetric(horizontal: Dimensions.PADDING_SIZE_SMALL),
                            child: Image.asset(Images.wish_list_delete, height: 25, width: 25)),
                        // SizedBox(width: Dimensions.PADDING_SIZE_DEFAULT),
                        Text( 'delete_account'.tr, style: DMSansMedium.copyWith(fontSize: Dimensions.fontSizeDefault, color: Theme.of(context).primaryColor)),
                        SizedBox(width: Dimensions.PADDING_SIZE_DEFAULT),
                      ],
                    ),
                  ),
                ...
                InkWell(
                  onTap: () {
                    if(Get.find<AuthController>().isLoggedIn()) {
                      Get.dialog(ConfirmationDialog(icon: Images.logout_dialog, description: 'are_you_sure_to_logout'.tr, isLogOut: true, onYesPressed: () async {
                        await Get.find<AuthController>().deleteToken();
                        Get.find<ProfileController>().clearProfileData();
                        Get.find<AuthController>().clearSharedData();
                        Get.find<AuthController>().isLoggedIn();
                        ...
                        Get.find<WishListController>().getWishList();
                        _isLoggedIn = false;
                        Get.find<CartController>().getCartList(notify: true);
                        Get.back();
                        Get.toNamed(RouteHelper.getSignInRoute());
                        }), useSafeArea: false);
                    }else {
                  ...
                  child: Padding(
                    ...
                    child: Container(
                        ...
                        child: Row(
                          children: [
                            Image.asset(Images.logout, height: 25, width: 25),
                            SizedBox(width: Dimensions.PADDING_SIZE_DEFAULT),
                            Text( _isLoggedIn ? 'logout'.tr : 'sign_in'.tr,
                                style: DMSansMedium.copyWith(fontSize: Dimensions.fontSizeDefault, color: Theme.of(context).primaryColor)),
                          ],
                        )),
                  ),
                ),
