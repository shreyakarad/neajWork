...
class GroupedProductView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<ProductController>(builder: (productController) {
      return (productController.groupedProduct != null && productController.groupedProduct.length == 0) ? SizedBox() :
      Column(
        children: [
          Padding(
            padding: EdgeInsets.fromLTRB(Dimensions.PADDING_SIZE_DEFAULT, 7, Dimensions.PADDING_SIZE_DEFAULT, 0),
            child: TitleWidget(
          ...
          Row(children: [
            Expanded(
              child: SizedBox(
                height: 170,
                child: productController.groupedProduct != null ? ListView.builder(
                  controller: _scrollController,
                  itemCount: productController.groupedProduct.length > 15 ? 15 : productController.groupedProduct.length,
                                ...
                                Positioned(
                                  ...
                                  child: Container(
                                    ...
                                    decoration: BoxDecoration(
                                      ...
                                      borderRadius: BorderRadius.only(bottomLeft: Radius.circular(Dimensions.RADIUS_DEFAULT), bottomRight: Radius.circular(Dimensions.RADIUS_DEFAULT)),
                                    ),
                                    child: Center(
                                      child: Text(productController.groupedProduct[index].name, style: poppinsRegular.copyWith(fontSize: Dimensions.fontSizeDefault, color: Theme.of(context).cardColor, overflow: TextOverflow.ellipsis)),
                                    ),
                                  )
                                ),
                              ]
                ...
                ) : GroupedProductShimmer(productController: productController),
              ),
            ),
          ],
          ),
