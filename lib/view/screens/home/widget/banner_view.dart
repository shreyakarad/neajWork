...
class BannerView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<BannerController>(builder: (bannerController) {
      ...
      (bannerController.bannerList != null && bannerController.bannerList.isNotEmpty) ?
      Container(
        width: MediaQuery.of(context).size.width,
        //height: 220,
        child: Swiper(
          itemBuilder: (BuildContext context, int index) {
            return ClipRRect(
              borderRadius: BorderRadius.circular(Dimensions.PADDING_SIZE_DEFAULT),
              child: Container(
                color: Get.isDarkMode ? Theme.of(context).colorScheme.primaryContainer : Theme.of(context).primaryColor,
                child: GetBuilder<AuthController>(builder: (splashController) {
                  return InkWell(
                    child: CustomImage(
                      image: '${bannerController.bannerList[index].image}',
                      fit: BoxFit.cover,
                    ),
                    onTap: () {
                      if (bannerController.bannerList[index].resourceType == 'product' &&  bannerController.bannerList[index].product != null) {
                        Get.toNamed(RouteHelper.getProductDetailsRoute(bannerController.bannerList[index].product, null, false));
                      } else if (bannerController.bannerList[index].resourceType == 'category' &&  bannerController.bannerList[index].category != null) {
                        Get.toNamed(RouteHelper.getCategoryProductRoute(CategoryModel(id: bannerController.bannerList[index].category)));
                      }},
                  );
                },
                ),
