...
import 'package:flutter_woocommerce/view/screens/payment/paypal/src/screens/complete_payment.dart';
...
class UsePaypal extends StatefulWidget {
  final Function onSuccess, onCancel, onError, onClose;
      ...
      onWillPop: () async {
        if (pressed < 2) {
          ...
          final snackBar = SnackBar(
              content: Text('Press back ${3 - pressed} more times to cancel transaction'));
                ...
                Expanded(child: Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(navUrl, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 14)),
                      ),
                      SizedBox(width: pageloading ? 5 : 0),
                      pageloading ? const Padding(padding: EdgeInsets.all(10), child: Center(child: CircularProgressIndicator())) : const SizedBox()
                    ],
                  ),
                ))
              ],
            ...
            child: loading
                ...
                : loadingError
                    ? Column(
                        children: [
                          Expanded(
                            child: Center(child: NetworkError(loadData: loadPayment, message: "Something went wrong,")),
                          ),
                        ],
                      )
                    : Column(
